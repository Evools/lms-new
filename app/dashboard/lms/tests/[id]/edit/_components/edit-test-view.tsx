...
export function EditTestView({
      ...
      prev.map((q, i) => {
        ...
        if (q.type === "MULTIPLE") {
          try {
            ...
            newCorrect = JSON.stringify([text]);
          }
        } else {
  ...
  const handleAddOption = (qIdx: number) => {
    setQuestionDrafts((prev) =>
      prev.map((q, i) => {
        if (i !== qIdx) return q;
        const newOptName = `Вариант ${q.options.length + 1}`;
  ...
  const handleToggleOptionCorrect = (qIdx: number, optionVal: string) => {
    setQuestionDrafts((prev) =>
      prev.map((q, i) => {
        if (i !== qIdx) return q;
        if (q.type === "MULTIPLE") {
          ...
          try {
            currentCorrect = JSON.parse(q.correctAnswer);
            if (!Array.isArray(currentCorrect)) currentCorrect = [];
          } catch {
            currentCorrect = [];
          }
          ...
          if (currentCorrect.includes(optionVal)) {
            ...
            updatedCorrect = currentCorrect.filter((item) => item !== optionVal);
          } else {
            updatedCorrect = [...currentCorrect, optionVal];
          }

          return { ...q, correctAnswer: JSON.stringify(updatedCorrect) };
        } else {
          return { ...q, correctAnswer: optionVal };
        }
      })
    );
  };

  const isOptionCorrect = (q: QuestionDraft, optionVal: string) => {
    if (q.type === "MULTIPLE") {
      try {
        const arr = JSON.parse(q.correctAnswer);
        return Array.isArray(arr) && arr.includes(optionVal);
      } catch {
        return false;
      }
    }
  ...
  const totalPoints = questionDrafts.reduce((sum, q) => sum + (q.points || 1), 0);
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-sm font-bold text-foreground flex items-center gap-2">
              <FileCheck2 className="h-4 w-4 text-primary" /> Редактирование теста
            </h1>
            <p className="text-[11px] text-muted-foreground">
              Изменение параметров, добавление и коррекция вопросов, способов проверки и баллов
            </p>
          </div>
        </div>
