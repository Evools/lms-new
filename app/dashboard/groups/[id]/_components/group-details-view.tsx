...
export function GroupDetailsView({ group, userRole, weeklyDays = [] }: GroupDetailsViewProps) {
  ...
  const handleAutoGenerateDuty = () => {
    setDutyActionStatus(null);
    startTransition(async () => {
      const res = await generateWeeklyDutyAction(group.id);
      if (res.success) {
        setDutyActionStatus({ success: "График дежурств на неделю сформирован!" });
        router.refresh();
        setTimeout(() => setDutyActionStatus(null), 3000);
      } else {
        setDutyActionStatus({ error: res.error || "Ошибка генерации ротации" });
      }
    });
  };
  ...
  return (
    <div className="w-full space-y-4 text-xs pb-16">
      ...
      <div className="space-y-4">
        ...
        {/* TAB 4: AUTOMATED & MANUAL DUTY SCHEDULE CALENDAR */}
        {activeTab === "DUTY" && (
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-muted/20 p-3 rounded-xl border">
              <div>
                <div className="text-xs font-bold text-foreground flex items-center gap-2">
                  ...
                  Календарный график дежурств
                </div>
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Автоматическая ротация и ручное назначение дежурных по дням
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                ...
                {isAdminOrTeacher && (
                  <Button
                    size="xs"
                    variant="outline"
                    onClick={handleAutoGenerateDuty}
                    disabled={isPending}
                    className="h-8 text-xs gap-1.5"
                  >
                ...
                <Button size="xs" variant="outline" className="h-8 text-xs gap-1.5" render={<Link href={`/dashboard/duty?group=${group.id}`} />}>
                  <Clock className="h-3.5 w-3.5" /> Управление
                </Button>
              </div>
            </div>
            ...
            {weeklyDays.length === 0 && (
              <div className="rounded-xl border border-dashed py-10 text-center text-muted-foreground space-y-2">
                ...
                {isAdminOrTeacher && (
                  <Button size="xs" onClick={handleAutoGenerateDuty} className="mt-2 text-xs">
